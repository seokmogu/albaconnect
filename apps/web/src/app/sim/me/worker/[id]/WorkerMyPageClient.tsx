          ...
          {activeTab === "일감" && (
            <section>
              ...
              {jobs.length === 0 ? (
                ...
                <ul className="space-y-3">
                  {jobs.map(({ dispatch, posting }) => (
                    <li key={dispatch.postingId} className="bg-white border border-[#EEEEEE] rounded-2xl p-4">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="font-bold text-[#1A1A1A] text-sm">{posting.draft.title}</h3>
                        <span className="flex items-center gap-1 text-xs text-[#22C55E] font-semibold">
                          <CheckCircle size={11} />확정
                        </span>
                      </div>
                      <p className="text-xs text-[#999999] mb-2">{posting.employerName}</p>
                      <div className="flex items-center gap-2">
                        <Banknote size={14} className="text-[#FF4D4D]" />
                        <span className="text-base font-black text-[#FF4D4D]">
                          {(posting.draft.hourlyRate * posting.draft.durationHours).toLocaleString()}원
                        </span>
